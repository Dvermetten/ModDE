...
class ModularDE:
    ...
    def mutate(self) -> None:
        ...
        parent_idxs = get_parent_idxs(
            self.parameters.population.n, self.parameters.min_lambda)
        mutated = np.zeros(self.parameters.population.x.shape)
        if self.parameters.mutation_base == 'rand':
            mutated += self.parameters.population[parent_idxs[:,
                                                              curr_parent_idx].tolist()].x
            curr_parent_idx += 1
            ...
            mutated += self.parameters.population[int(
                np.argmin(self.parameters.population.f))].x
        ...
        if self.parameters.mutation_reference == 'pbest':
            if self.parameters.pbest_value:
                pbest = np.random.uniform(
                    0, self.parameters.pbest_value, self.parameters.population.n)
            else:
                pbest = np.random.uniform(
                    2/self.parameters.population.n, 0.2, size=self.parameters.population.n)
            idxs_pbest = np.argsort(self.parameters.population.f)[np.random.randint(
                np.clip(self.parameters.population.n*pbest, 1, self.parameters.population.n))]
            mutated += F * \
                (self.parameters.population[idxs_pbest.tolist(
                )].x - self.parameters.population.x)
        elif self.parameters.mutation_reference == 'best':
            mutated += F * (self.parameters.population[int(
                np.argmin(self.parameters.population.f))].x - self.parameters.population.x)
        elif self.parameters.mutation_reference == 'rand':
            mutated += F * \
                (self.parameters.population[parent_idxs[:, curr_parent_idx].tolist(
                )].x - self.parameters.population.x)
            curr_parent_idx += 1

        if self.parameters.use_archive and self.parameters.archive is not None:
            archive_idxs = np.random.randint(
                self.parameters.archive.n, size=self.parameters.population.n)
            mutated += F * (self.parameters.population[parent_idxs[:, curr_parent_idx].tolist(
            )].x - self.parameters.archive[archive_idxs.tolist()].x)
            curr_parent_idx += 1
            n_comps_to_add -= 1

        for mut_idx in range(n_comps_to_add):
            mutated += F * (self.parameters.population[parent_idxs[:, curr_parent_idx].tolist(
            )].x - self.parameters.population[parent_idxs[:, curr_parent_idx+1].tolist()].x)
    ...
    def select(self) -> None:
        ...
        """
        x_sel = np.where(self.parameters.population.f < self.parameters.offspring.f,
                         self.parameters.population.x, self.parameters.offspring.x)
        f_sel = np.where(self.parameters.population.f < self.parameters.offspring.f,
        ...
        self.parameters.improved_individuals_idx = np.where(
            self.parameters.population.f < self.parameters.old_population.f)[0]
    ...
    def crossover(self) -> None:
        ...
        if self.parameters.crossover == "bin":
            ...
            j_rand = np.random.randint(
                0, self.parameters.population.x.shape[0], size=self.parameters.population.x.shape[1])
            chosen[j_rand.reshape(-1, 1), np.arange(
                self.parameters.population.x.shape[1])[:, None]] = 0
            ...
            for ind_idx in range(self.parameters.population.n):
                ...
                while offset < self.parameters.population.d:
                    ...
                    crossed[(k+offset) % self.parameters.population.d,
                            ind_idx] = mutated[(k+offset) % self.parameters.population.d, ind_idx]
                    offset += 1
    ...
    def fitness_func(self, x: np.ndarray) -> float:
        ...
        if self.parameters.init_stats:
            ...
            if self.parameters.stats.corrected:
                ...
                x_transformed = (
                    (x - self.parameters.lb) / (self.parameters.ub - self.parameters.lb)).flatten()
                ...
                self.parameters.stats.ED = float(
                    np.linalg.norm(x_transformed-x_pre))
            else:
...
#         print(new_x)
    ...
    def correct_bounds(self) -> np.ndarray:
        ...
        if self.parameters.bound_correction in ['hvb', 'expc_target', 'vector_target']:
            ...
            base_x = (self.parameters.population.x[np.argmin(
                self.parameters.population.f)] - self.parameters.lb) / (self.parameters.ub - self.parameters.lb)
        else:
            base_x = None
...
def perform_correction(x_transformed, oob_idx, method, base_vector=None):
    ...
    if method == "mirror":
        ...
        x_transformed[oob_idx] = np.abs(2*(y > 0) - ((y > 0)-np.log(1+np.random.uniform(
            size=np.sum(oob_idx))*(np.exp(-1*np.abs((y > 0) - base_vector[oob_idx]))-1))))
